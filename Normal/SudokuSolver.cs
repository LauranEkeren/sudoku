namespace Normal {
    class SudokuSolver{
        ...
        public NormalSudoku startSolving(NormalSudoku sudoku){
            Console.WriteLine("Start solving");

            return solveSudoku(sudoku, 0);
        }

        private NormalSudoku solveSudoku(NormalSudoku sudoku, int position){
                ...
                if(sudoku.getSpace(position).getNumber() == 0){
                    ...
                    while (counter <= 9){
                        ...
                        if(sudoku.isSudokuValid()){
                            NormalSudoku copy = sudoku;
                            NormalSudoku newSudoku = solveSudoku(copy, position + 1);
